...
export default function Dashboard() {
  ...
  const [announcement, setAnnouncement] = useState<{ content: string; enabled: boolean; level: 'info' | 'warning' | 'alert' } | null>(null)
  ...
  const loadAnnouncement = async () => {
    try {
      const data = await apiPost<any>('/system/announcement')
      if (data) setAnnouncement(data)
    } catch { }
  }

  const handleRefill = async () => {
    if (!refillCode.trim()) return
    try {
      setRefillLoading(true)
      const res = await apiPost<{ message: string; profile?: AuthProfile }>('/auth/refill', { code: refillCode })
      alert(res?.message || '续期成功')
      setRefillOpen(false)
      setRefillCode('')
      if (res?.profile) setAuthProfile(res.profile)
      else loadAuthProfile()
    } catch (e: any) {
      alert(e.message || '续期失败')
    } finally {
    ...
    <div className="app retro-bg">
      ...
      <div className="retro-shell">
        {announcement?.enabled && announcement.content ? (
          <div className={`retro-announcement ${announcement.level || 'info'}`} style={{
            background: announcement.level === 'alert' ? '#ff0000aa' : announcement.level === 'warning' ? '#ff8800aa' : '#0088ffaa',
            padding: '8px', marginBottom: '10px', borderRadius: '4px', textShadow: '1px 1px 0 #000', border: '1px solid rgba(255,255,255,0.3)'
          }}>
        ...
        <div className="retro-panel retro-card-info">
          ...
          <button className="retro-setting-link" onClick={() => setExpansionOpen(true)} style={{ marginLeft: '10px', textDecoration: 'underline', cursor: 'pointer', background: 'none', border: 'none', color: '#fff' }}>
            [扩容]
          </button>
          <button className="retro-setting-link" onClick={() => setRefillOpen(true)} style={{ marginLeft: '10px', textDecoration: 'underline', cursor: 'pointer', background: 'none', border: 'none', color: '#fff' }}>
            [续期]
          </button>
        </div>
      ...
      {refillOpen ? (
        <div className="retro-modal-mask">
          <div className="retro-panel retro-modal">
            <h3>卡密续期</h3>
            <div className="retro-modal-body">
              <p style={{ marginBottom: '10px' }}>当前有效期至: {cardExpireText}</p>
              <p style={{ marginBottom: '15px', color: '#aaa' }}>使用同一类型的时长卡（或测试卡）已延长有效期。</p>
            ...
            <div className="retro-modal-actions">
              <button className="retro-auth-btn" onClick={handleRefill} disabled={refillLoading}>
                {refillLoading ? '续期中...' : '确认续期'}
              </button>
              <button className="retro-auth-btn secondary" onClick={() => setRefillOpen(false)}>
                取消
              </button>
            </div>
          </div>
